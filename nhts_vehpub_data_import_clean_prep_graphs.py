...
bestmile = pd.DataFrame(bestmile.groupby(['fuel_decile'])['BESTMILE'].mean().reset_index())
bestmile_mean = bestmile['BESTMILE'].mean()

bestmile.to_excel(r'C:\Users\HP Envy\Dropbox\Gas_Subsidy_Model\3_Output\bestmile.xlsx')
...
location_usage_total = pd.DataFrame(location_usage.groupby(['HBHUR']).size().reset_index())
...
superusers = pd.DataFrame(superusers.groupby(['HOUSEID']).size().reset_index(name='count'))
superusers = superusers[superusers['count'] > 1]
